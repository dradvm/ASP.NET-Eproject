...
namespace ABCDMall.Controllers
{
    public class FoodcourtsController : Controller
    {
        ...
        public ActionResult Add(int status = 1)
        {
            ...
            var shopTypes = db.ShopTypes.ToList(); // Lấy danh sách các ShopType từ database
        ...
        public ActionResult Update(int id, string name, HttpPostedFileBase logo, int shopTypeID, int floor, string description)
        {
            ...
            {
                return Redirect($"/foodcourts/update/{id}?status=0"); // Kiểm tra tên cửa hàng
            }
            shop.Name = name.Trim();

            if (string.IsNullOrEmpty(description?.Trim()))
            {
                return Redirect($"/foodcourts/update/{id}?status=-6"); // Kiểm tra mô tả
            }
            shop.Description = description.Trim();
            ...
            if (logo != null && logo.ContentLength > 0)
            {
                ...
                if (!supported.Any(format => string.Equals(format, Path.GetExtension(logo.FileName), StringComparison.OrdinalIgnoreCase)))
                {
                    return Redirect($"/foodcourts/update/{id}?status=-2"); // Kiểm tra định dạng ảnh
                }

                if (logo.ContentLength > 10000000)
                {
                    return Redirect($"/foodcourts/update/{id}?status=-3"); // Kiểm tra kích thước ảnh
                }
            ...
            return Redirect("/foodcourts/index"); // Điều hướng về trang danh sách
        }
